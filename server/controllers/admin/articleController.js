...
const dayjs = require('dayjs') // 时间格式化插件
...
module.exports = {
  ...
  getArticleDetail: async ctx => {
    ...
    if (result) {
      result.dataValues.category = result.dataValues.my_news_type.dataValues.type_name
      delete result.dataValues.my_news_type
      ctx.body = {
        status: 200,
        msg: '获取成功',
        data: {
  ...
  getArticleList: async (ctx) => {
    ...
    if (count > 0) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        data.update_time = dayjs(date).format('YYYY-MM-DD')
        data.type = data.my_news_type.dataValues.type_name
        delete data.my_news_type
