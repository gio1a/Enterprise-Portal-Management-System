...
module.exports = {
  ...
  newsList: async (ctx) => {
    ...
    if (count > 0) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        rows[i].dataValues.publish_time = dayjs(date).format('YYYY-MM-DD')
  ...
  hotNews: async (ctx) => {
    ...
    if (result.length > 0) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        result[i].dataValues.publish_time = dayjs(date).format('YYYY-MM-DD')
  ...
  recommendNews: async (ctx) => {
    ...
    if (result.length > 0) {
      const recommendNews = []
      for (let i = 0; i < result.length; i++) {
        const data = result[i].dataValues.my_new.dataValues
