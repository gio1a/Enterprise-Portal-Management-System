...
module.exports = {
  ...
  getJobDetail: async ctx => {
    ...
    if (result) {
      ...
      temp.publish_time = dayjs(publish_time).format('YYYY-MM-DD')
      temp.update_time = dayjs(update_time).format('YYYY-MM-DD')
      delete temp.create_time
      const jobDetail = {
        title: temp.title,
        city: temp.my_city_info.dataValues.name,
        job_category: temp.my_job_category.dataValues.name,
